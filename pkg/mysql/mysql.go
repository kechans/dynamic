...
func (db *MysqlBaseDao) jannaWatch(callName, targetTag string) {
	...
	for mysqlEvent := range ch {
		logkit.Infof("watch event Tag:%s, Address:%s, Port:%d, Master:%s, Id:%s, Opt:%s", mysqlEvent.Tag, mysqlEvent.Address, mysqlEvent.Port, mysqlEvent.Master, mysqlEvent.Id, mysqlEvent.Opt)
		var conf *Config
		var serverPool *[]connPool
